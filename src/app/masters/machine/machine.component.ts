...
export class MachineComponent implements OnInit {
 ...
 // noSpacePattern = "^(?=.*[0-9]\.\d)(?=.*[a-zA-Z])([a-zA-Z0-9]+)$";
  ...
  ngOnInit() {
    ...
    this.userRecord = JSON.parse(localStorage.getItem('user'));
   {
    ...
    this.form = this.formBuilder.group({
      ...
      smartBatch: [null, Validators.required],
      // smartBatchType: [null, Validators.required],
      smartBatchType: [{ value: null, disabled: true }, Validators.required],
      createdBy: [''],
      updatedBy: ['']
    });

    // Subscribe to smartBatch changes
  this.form.controls['smartBatch'].valueChanges.subscribe(value => {
    this.onSmartBatchChange(value);
  });
  ...
  onSmartBatchChange(event) {
    const smartBatchValue = this.form.controls['smartBatch'].value;
    const smartBatchTypeControl = this.form.controls['smartBatchType'];
  
    if (smartBatchValue === 'false') {
      smartBatchTypeControl.setValue('Normal CBC');
      smartBatchTypeControl.disable();
    } else {
      smartBatchTypeControl.setValue(null);
      smartBatchTypeControl.enable();
    }
  }

  onSubmit() {
    this.submitted = true;
    // Enable smartBatchType control before submitting
    this.form.controls['smartBatchType'].enable();
    ...
    // // If smartBatch is false, remove smartBatchType key
    // if (this.form.controls['smartBatch'].value === 'false') {
    //   this.form.removeControl('smartBatchType');
    // } else {
    //     // Ensure the smartBatchType control is present
    //     if (!this.form.contains('smartBatchType')) {
    //         this.form.addControl('smartBatchType', this.formBuilder.control(null, Validators.required));
    //     }
    // }
    ...
    // Re-add smartBatchType control after submission if it was removed
    if (!this.form.contains('smartBatchType')) {
      this.form.addControl('smartBatchType', this.formBuilder.control(null, Validators.required));
    }
  }
